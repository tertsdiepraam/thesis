...
In the program above, \el{f} will not get called because \el{hAbort} does not call the continuation, but \el{g} will be called, because it is used outside the \el{handle} construct.
...
\TODO{Remove all mentions of macros, they're too distracting and do not actually explain anything}

\feedback[inline]{Val as a higher-order effect is weird, because there's no reason for it to be HO. Change it to something actually HO.}
...
\feedback{Listings should probably be in figures and not break over pages.}
