...
However, the ergonomics of these libraries depend on the capabilities of the type system of the language.   Since the effects are encoded as a monad, a monadic style of programming is still required. For both signatures of \code{readfile} above, using the function and manipulating the output looks the same. For example, a function that reads the first line from a file might be written as below.
