...
\TODO{This should not be the Val effect, maybe local?}
...
This is a nice convenience, but it requires some caution. A problem arises when we define multiple elaborations for an effect; which one should then be used? We have chosen to simply give a type error in this case.\feedback{Explain why \& how that makes it deterministic.} To fix the type error we simply provide the elaboration we want with \el{elab[eVal1]}.
