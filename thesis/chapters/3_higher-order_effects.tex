...
Elaine's model for higher-order effects, which is explained in \cref{sec:hoeffects}, ignores this constraint. Instead, Elaine allows for elaboration of single higher-order effects at a time, much like handlers in algebraic effects. We believe this to be semantically equivalent to hefty algebras. In the prototype, this variation has also not presented any problems. However, we do not have a formal argument to show that this is equivalent to hefty algebras as described above. This is a gap in theory that future research can fill.
