...
\feedback{Casper: clarify why not, omit, or replace by a statement that it is a prototype/intended as a demonstration/vehicle for research/... if that is what you mean.}
...
\begin{itemize}
    \item We present a language with both handlers and elaborations based on hefty algebras (\cref{chap:basics}). The language definition consists of a syntax definition, typing judgments and reduction semantics. (\cref{chap:spec}). We conjecture that the semantics of the language are equivalent\feedback{"give a semantics", "hefty algebras are the semantics of"} to hefty algebras, but we do not proof soundness.
    %
    \item This language includes a type system for a language with higher-order effects and elaborations. This type system generalizes effect rows from languages with algebraic effects to higher-order effects.\feedback{Last bit most important}
    %
    \item We provide a prototype with a parser, type checker and interpreter for this language implemented in Haskell. This prototype is available in the artifact.\feedback{Mention Hindley-Milner here}
    %
